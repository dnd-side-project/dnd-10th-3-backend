...
public class CommentController {
    ...
    @GetMapping("/{voteId}/all")
    @Operation(summary = "댓글 전체 조회 ", description = "인증된 회원이 댓글을 전체 조회합니다.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "조회 성공"),
            @ApiResponse(responseCode = "404", description = "조회 실패", content = @Content(
                    mediaType = "application/json",
    ...
    ResResult<List<CommentResponseDto>> getAllComments(@PathVariable(name = "voteId") Long voteId,
                                                       Authentication authentication, HttpServletResponse response) {
        return ResponseCode.COMMENT_READ.toResponse(
                commentService.findAllComments(authentication.getName(),voteId),response);
    }
}
